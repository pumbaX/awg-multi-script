...
Jc = 3–7 (AWG 1.5/2.0) или 4–7 (AWG 1.0) — оптимальный баланс

Jmin — генерируется случайно (64–256) для повышения энтропии

H1–H4 всегда ≥5 — чтобы не конфликтовать со стандартными WireGuard-заголовками
...
Все сетевые вызовы имеют таймауты — скрипт не зависает при проблемах с сетью

При создании нового сервера скрипт предлагает удалить старых клиентов (если они есть)
